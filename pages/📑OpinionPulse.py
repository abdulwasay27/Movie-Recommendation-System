...
def fetch_movie_reviews(movie_id):
    ...
    url = f"https://www.imdb.com/title/{movie_id}/reviews/?ref_=tt_ururv_sm&sort=num_votes"
    response = requests.get(url, headers=HEADERS)
    ...
    if response.status_code != 200:
        ...
        try:
            soup = BeautifulSoup(response.text, "html.parser")
            review_containers=soup.findAll("div", class_=["ipc-list-card__content"])
            total_reviews = []
            content = {}
            for single_review_container in review_containers:
                spoiler_button = single_review_container.find("button", class_=["review-spoiler-button"])
    
                if spoiler_button==None:
                    title=single_review_container.find("a", class_=["ipc-title-link-wrapper"])
                    review=single_review_container.find("div", class_=["ipc-html-content-inner-div"])
